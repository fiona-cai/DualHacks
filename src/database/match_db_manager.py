...
def get_match_by_id(match_id):
    ...
    results = list(db.collection("matches").where(filter=FieldFilter("match_id", "==", match_id)).limit(1).stream())
    if len(results) == 0:
        return None
...
def add_match(match):
    ...
    doc_ref = db.collection("matches").document(match.match_id)
...
def get_last_match_id():
    ...
    results = list(db.collection("matches").order_by("match_id").limit(1).stream())
    if len(results) == 0:
        return 0

    loaded_match = results[-1].to_dict()
...
def get_match_players(match_id):
    ...
    results = list(db.collection("matches").document(match_id).collection("players").stream())
...
def add_player_to_match(match_id, player):
    ...
    doc_ref = db.collection("matches").document(match_id).collection("players").document(player.username)
...
def add_to_available_match(player):
    ...
    loaded_match = results[0].to_dict()
    match = Match(**loaded_match).match_id
